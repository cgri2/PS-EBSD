...
import numpy as np
from kikuchipy.signals.util._master_pattern import (_project_single_pattern_from_master_pattern,                                                   _get_direction_cosines_from_detector)
...
"""
def EBSD_subset(xpat, det, xmap, n_points=None, indices=None):
    Extract an evenly spaced NyS x NxS subset from EBSD data,
    Or extracts subset based on indices provided.

    Conventions:
    - Navigation shape: (Ny, Nx) = (rows=y, cols=x)
    - Pattern shape: (py, px)
    - xpat.data shape: (Ny, Nx, py, px)
    - xmap.rotations, xmap.phase_id, xmap.x, xmap.y are 1D of length Ny*Nx
    - det navigation may be 1D (N) or 2D (Ny, Nx)

    Input:
    - xpat: full pattern dataset
    - det: detector with pc values for each point
    - xmap: crystal map with orientations
    Options:
    - n_points: number of evenly spaced points (n_points_y, n_points_x) in each dimension
    - indices: explicit list of 1D indices

    Returns:
      xpatS: EBSD signal with shape (NyS, NxS, py, px)
      detS: EBSDDetector with pc for the subset
      xmapS: CrystalMap for the subset

    # Define original data shape and indices
    Ny, Nx, py, px = xpat.data.shape
    N = Ny * Nx
    idx_2D = np.arange(N).reshape(Ny, Nx)
        
    # equally spaced grid
    if n_points is not None and indices is None:
        n_points_y, n_points_x = map(int, n_points)
        ys = np.unique(np.linspace(0, Ny - 1, n_points_y, dtype=int))
        xs = np.unique(np.linspace(0, Nx - 1, n_points_x, dtype=int))
        NyS, NxS = len(ys), len(xs)
        flat_idx_list = idx_2D[np.ix_(ys, xs)].ravel().tolist()
        out_shape = (NyS, NxS)                 # grid output
        idx_arr = np.asarray(flat_idx_list, dtype=int).ravel()
        ...
        step_y = (ys[1] - ys[0])*xmap.dy
        step_x = (xs[1] - xs[0])*xmap.dx
        coords, _ = create_coordinate_arrays(shape=out_shape, step_sizes=(step_y, step_x))  # coords["x"], coords["y"] are length NyS*NxS
        ...
        print('xmapS shape: ', xmapS.shape)

    #provided indices
    if indices is not None:
        idx_arr = np.asarray(indices, dtype=int).ravel()
        flat_idx_list = idx_arr.tolist()
        K = idx_arr.size
        if K % 2 != 0:
            raise ValueError("For a (N/2, 2) grid, number of indices K must be even.")
        #Create fake x, y positions to make xmap shape match
        NyS = K // 2
        NxS = 2
        out_shape = (NyS, NxS)
        xs = np.arange(NxS)           # [0, 1]
        ys = np.arange(NyS)           # [0, 1, 2, ..., NyS-1]
        x_grid, y_grid = np.meshgrid(xs, ys)   # both (NyS, NxS)
        x_fake = x_grid.ravel()       # length K = NyS*NxS
        y_fake = y_grid.ravel()       # length K
        xmapS = CrystalMap(
            rotations=xmap.rotations[idx_arr],
            phase_id=xmap.phase_id[idx_arr],
            x=x_fake,
    ...
    # --- Select patterns using unfold() -> 1D navigation, then keep order ---
    x1d = xpat.deepcopy()
    x1d.unfold()                              # in-place, 1D nav of length N
    xpat_sel = x1d.inav[(flat_idx_list,)].deepcopy()
    xpat_sel.compute()
    data = np.asarray(xpat_sel.data).reshape(NyS, NxS, py, px)
    xpatS = kp.signals.EBSD(data)
    print('xpatS shape: ', xpatS.data.shape)

    # --- Subset detector PC; handle 1D or 2D navigation shapes robustly ---
    def _sel_nav(arr, flat_idx_list, Nx, out_shape=None):
        
        #Select navigation elements from arr using flat_idx_list.
        #- If arr.ndim == 2: treat as (Ny, Nx) and gather via iy, ix.
        #- If arr.ndim == 1: gather directly.
        #Reshape to out_shape if provided (grid); otherwise keep 1D.
        
        a = np.asarray(arr)
        idx = np.asarray(flat_idx_list, dtype=int)
        if a.ndim == 2:
            iy = idx // Nx
            ix = idx %  Nx
            sel = a[iy, ix]
            ...
            raise ValueError("Detector PC array must be 1D or 2D over navigation.")
        if out_shape is not None:
            sel = sel.reshape(*out_shape)
        return sel

    pcx = _sel_nav(det.pcx, flat_idx_list, Nx, out_shape)
    pcy = _sel_nav(det.pcy, flat_idx_list, Nx, out_shape)
    pcz = _sel_nav(det.pcz, flat_idx_list, Nx, out_shape)
    pc = np.stack([pcx, pcy, pcz], axis=-1)   # shape: (K, 3) or (NyS, NxS, 3)
    ...
    print('detS nav shape: ', detS.navigation_shape)

    return xpatS, detS, xmapS
"""
